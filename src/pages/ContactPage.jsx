  ...
  return (
    <div className="page-enter page-enter-active">
      ...
      <section className="density-sect max-w-[1200px] mx-auto px-6 md:px-12 lg:px-20 grid md:grid-cols-3 gap-10">
        <div><Kicker>WhatsApp & phone</Kicker><p className="font-mono text-lg text-night mt-4">{VENUE.whatsapp}</p><p className="text-sm text-ink/70 mt-2">Reservations, rides home, feedback.</p></div>
        <div><Kicker>Hours</Kicker><ul className="mt-4 space-y-2 text-sm text-ink/80">{VENUE.hours.map((h) => <li key={h.label}><b className="text-night">{h.label}</b><br />{h.time}</li>)}</ul></div>
        <div><Kicker>Address</Kicker><p className="mt-4 text-sm text-ink/80 leading-relaxed">{VENUE.address}</p><button onClick={() => onNavigate("getting-here")} className="mt-3 font-mono text-[10px] uppercase tracking-[0.2em] text-brass hover:text-night">Getting here →</button></div>
